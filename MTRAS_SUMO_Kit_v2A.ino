/* MTRAS Kit v2.A SUMO Code
2/3/2024
Cleaned up code.
Andrew H helped me get "Forward" function going by teaching me more about how the "While" and "int++"statement works.
...
int irspeedFor = 65;  //Initial right motor speed

//Left Motor
int SpeedupL;         //Left motor ramp up int
int lspeedFor = 65;   //Left motor speed
int ilspeedFor = 65;  //Initial left motor speed
...
void loop() {
  ...
  while (buttonSelectState == HIGH) {  //While the button is not pressed do the following
    ...
    if ((potValueFL < 1500) && (potValueFL > 100)) {
      reverseR();
    }
    if ((potValueFR < 1500) && (potValueFR > 100)) {
      reverseL();
    }
  }
...
void forward() {
  ...
  display.display();
  readsensors();
  while ((fl < 600 && fl > 20) && (fr < 600 && fr > 20)) {
    if (SpeedupR > 250) {
      SpeedupR = (250);
    }
    if (SpeedupL > 250) {
...
void bootscreen() {
  ...
  display.println(F("Andrews Robot"));     //Replace "Your" with your name
  display.display();
  delay(1500);
  display.clearDisplay();
