...
char name[] = "Your   Robot"; //Change boot text to your name etc. 
...
//Motor Speeds
//
int turnspeed = 65;  //Adjust for turning speed
// Adjust for Reverse speed. Allows for adjustment of each motor. This makes it possible to adjust to how straight the robot goes.
int rspeedRev = 135;
int lspeedRev = 135;
// Adjust for Forward speed. Allows for adjustment of each motor. This makes it possible to adjust to how straight the robot goes.
//Right Motor
int SpeedupR;         //Right motor ramp up int
int rspeedFor = 65;   //Right motor speed
int irspeedFor = 45;  //Initial right motor speed
int rfmap;

//Left Motor
int SpeedupL;         //Left motor ramp up int
int lspeedFor = 65;   //Left motor speed
int ilspeedFor = 45;  //Initial left motor speed
int lfmap;
...
void loop() {
  ...
  while (buttonSelectState == HIGH) {  //While the button is not pressed do the following
    ...
    readsensors();
    delay(20);
    potValueFLOffset = potValueFL - 151;
    potValueFROffset = potValueFR - 0;
    rfmap = map(potValueFLOffset, 1200, 4095, 60, 0); //Only adjust the 3rd number for speed
    lfmap = map(potValueFROffset, 1200, 4095, 65, 0); //Only adjust the 3rd number for speed
  ...
  //Button was pressed so Stop motors and go back to waiting for a button press
  motorR.motorStop();
  motorL.motorStop();
  delay(1500);
  selectButton();
...
void go() {
  if (rfmap < 40) {
    motorL.motorRev(rfmap);
  }
  else {
      motorL.motorGo(rfmap);
}
if (lfmap < 40) {
  motorR.motorRev(lfmap);  //Forward
}
else {
    motorR.motorGo(lfmap);  //Forward
}
...
void bootscreen() {
  ...
  display.println(F(name));  //Replace "Your" with your name
  display.display();
  delay(1500);
  display.clearDisplay();
  display.setCursor(13, 0);              // Start at top-left corner
  display.println(F("Line Following"));  //Replace "Your" with your name
  display.display();
  delay(1500);
  display.clearDisplay();
