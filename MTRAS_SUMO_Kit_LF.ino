...
//Motor Speeds
//
int turnspeed = 50;  //Adjust for turning speed
...
void loop() {
  ...
  while (buttonSelectState == HIGH) {  //While the button is not pressed do the following
    ...
    rfmap = map(potValueFLOffset, 0, 3000, 65, 0); //Only adjust the 3rd number for speed
    lfmap = map(potValueFROffset, 0, 3000, 70, 0); //Only adjust the 3rd number for speed
...
void bootscreen() {
  ...
  display.println(F("Your Robot"));  //Replace "Your" with your name
  display.display();
  delay(1500);
  display.clearDisplay();
  display.setCursor(13, 0);              // Start at top-left corner
